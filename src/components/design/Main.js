  ...
  const media_url = () => {
    ...
    switch (props.data.type) {
      ...
      case 'blog':
        //invalid url check
        if (props.data.blogUrl.indexOf('http') === -1)
          return <img src={getImageUrl()} className={styles.img} alt={title} />

        return (
          <a href={props.data.blogUrl} target="_blank" rel="noopener noreferrer">
            <img src={getImageUrl()} alt={title} className={styles.img} />
          </a>
        )
      default:
        return <img src={getImageUrl()} alt={title} className={styles.img} />
    }
  }
