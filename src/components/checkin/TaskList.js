...
const TaskList = () => {
  ...
  const getProgress = taskId => {
    ...
    switch (taskId) {
      case '1axAkTN3zc': // Like users designs
        return determineProgress(numberLikes || 0, 30) // These values may not be in the task record if they would be at zero, so it's necessary to replace undefined with zero.
      case 'kmEXaEWYFG': // Follow Other Designers
        return determineProgress(numberFollows || 0, 30)
      ...
      case 'Xr4CyGw6Jb': // VirtualGift
        return determineProgress(numberGifts || 0, 20)
      case '8GORtOEbeF':
  ...
  let taskElements = dailyTasks.map(task => {
    if (task.objectId === 'wqdJDrrozL') return null // daily reward task is no longer used, and virtual gift is disabled until feature is complete.
    const claim = e => {
      e.stopPropagation()
